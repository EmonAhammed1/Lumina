...
theme.SwiperCustom = (function() {
  function SwiperCustom(container) {
    ...
    var isLoop = $(container).data('loop') || false;
...
theme.compare = (function (){
      ...
      for (var i = 0; i < compareObject.length; i++) { 
        ...
        Shopify.getProduct(productHandle,function(product){
          var htmlProduct = '',
              ...
              productTypeHTML = product.type !== "" ? '<span>'+ product.type +'</span>' : '<span>'+ theme.strings.none +'</span>',
              productVendorHTML = product.vendor !== "" ? '<span>'+ product.vendor +'</span>' : '<span>'+ theme.strings.none +'</span>',
              productTagsHTML = product.tags && product.tags.length > 0 ? '<span>' + product.tags.join(', ') + '</span>' : '<span>N/A</span>';
              
          var productImage = product.featured_image ? Shopify.resizeImage(product.featured_image,'x230') : '';
          
          htmlProduct += '<div class="compare-item col-6 col-md-6 px-2 mb-4">';
          htmlProduct += '  <div class="compare-card bg-white h-100 shadow-sm rounded p-3 text-center" style="position: relative; border: 1px solid #eaeaea; display: flex; flex-direction: column;">';
          
          // Proper Remove Button
          htmlProduct += '    <button class="js-remove-compare shadow-sm" data-handle="'+product.handle+'" style="position: absolute; top: -12px; right: -12px; z-index: 99; border-radius: 50%; width: 30px; height: 30px; padding: 0; display: flex; align-items: center; justify-content: center; background-color: #cc0000; color: #ffd700; border: 2px solid #fff; cursor: pointer; outline: none; box-shadow: 0 2px 5px rgba(0,0,0,0.2);" title="'+ theme.strings.compareTextRemove +'">';
          htmlProduct += '      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ffd700" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="margin:0; padding:0; display:block; position:static; transform: none;"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>';
          htmlProduct += '    </button>';
          
          // Image
          htmlProduct += '	<a class="d-flex align-items-center justify-content-center mb-3 mt-4" style="height: 180px;" href="'+ product.url +'">';
          if (productImage) {
            htmlProduct += '		<img src="'+ productImage +'" style="max-height: 100%; max-width: 100%; object-fit: contain;"/>';
          }
          htmlProduct += '	</a>';
          
          // Title
          htmlProduct += '	<h5 class="product-title mb-2 text-dark" style="font-size: 1rem; font-weight: 600; line-height: 1.4;">'+ product.title +'</h5>';
          
          // Price
          htmlProduct += '	<div class="product-price mb-3" style="font-size: 1.1rem; color: #333;">';
          if (productComparePrice) {
            htmlProduct += '	  <s class="text-muted small mr-2">'+ productComparePrice +'</s>';
          }
          htmlProduct += '	  <span class="font-weight-bold">'+ productPrice +'</span>';
          htmlProduct += '	</div>';
          
          // Status (using inline styles to guarantee it looks like a badge)
          htmlProduct += '	<div class="product-status mb-3">';
          if (product.available === true || product.available === "true") {
            htmlProduct += '    <span style="background-color: #28a745; color: white; border-radius: 4px; padding: 4px 10px; font-size: 0.85rem; font-weight: 600;">'+ theme.strings.available +'</span>';
          } else {
            htmlProduct += '    <span style="background-color: #dc3545; color: white; border-radius: 4px; padding: 4px 10px; font-size: 0.85rem; font-weight: 600;">'+ theme.strings.unavailable +'</span>';
          }
          htmlProduct += '  </div>';
          
          // Meta info
          htmlProduct += '  <div class="p-3 bg-light rounded text-left mt-auto">';
          htmlProduct += '	  <div class="product-meta text-muted small d-flex flex-column mb-2"><strong>Beneficii Cheie:</strong> <span class="mt-1">'+ productTagsHTML + '</span></div>';
          htmlProduct += '  </div>';
          
          htmlProduct += '  </div>'; // End compare-card
          htmlProduct += '</div>'; // End compare-item
          
          if (compareObject.length === 1) {
            htmlProduct += '<div class="compare-item col-6 col-md-6 px-2 mb-4 d-flex align-items-stretch justify-content-center">';
            htmlProduct += '  <div class="text-center p-4 w-100 d-flex flex-column justify-content-center align-items-center" style="border: 2px dashed #ccc; border-radius: 8px; background: #fafafa; min-height: 350px;">';
            htmlProduct += '    <div class="mb-3" style="color: #ccc;"><svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="16"></line><line x1="8" y1="12" x2="16" y2="12"></line></svg></div>';
            htmlProduct += '    <h5 class="mb-2 text-dark font-weight-bold" style="font-size: 1.1rem;">Adaugă încă un produs</h5>';
            htmlProduct += '    <p class="text-muted mb-4" style="font-size: 0.9rem;">Alege cel puțin încă un produs pentru a-l compara cu acesta.</p>';
            htmlProduct += '    <button type="button" class="btn btn-dark px-4 py-2" onclick="openProductSelector()">Caută produse</button>';
            htmlProduct += '  </div>';
            htmlProduct += '</div>';
          }
  ...
  $(document).on('click',compareButtonClass,function (event) {
    ...
    // Automatically open the comparison modal if the product was added
    var productHandle = $(this).data('handle');
    if ($.inArray(productHandle, compareObject) !== -1) {
  ...
  // Global animation functions for custom product selector overlay
  window.openProductSelector = function() {
    $('.modal-dialog--compare').addClass('is-shrunk');
    $('#jsProductSelector').addClass('is-active');
    
    var $list = $('#jsProductSelector').find('.js-compare-product-list');
    $list.html('<div class="col-12 text-center py-4"><div class="spinner-border text-dark" role="status"></div></div>');
    ...
    $.getJSON('/products.json', function(data) {
      ...
      for (var i = 0; i < products.length; i++) {
        ...
        if ($.inArray(product.handle, compareObject) !== -1) continue;
        
        var productImage = product.images && product.images.length > 0 ? product.images[0].src : '';
        if (productImage) {
          productImage = productImage.replace(/(\.[^.]*(?:\?.*)?)$/, '_150x$1'); 
        }
        
        var html = '<div class="col-6 col-md-4 mb-3">';
        html += '<div class="card h-100 shadow-sm text-center p-2" style="border: 1px solid #eaeaea;">';
        if (productImage) {
          html += '<img src="' + productImage + '" class="card-img-top mx-auto mt-2" style="max-height: 100px; width: auto; object-fit: contain;" alt="">';
        }
        html += '<div class="card-body p-2 d-flex flex-column">';
        html += '<h6 class="card-title text-dark mb-2" style="font-size: 0.85rem; font-weight: 600;">' + product.title + '</h6>';
        html += '<p class="text-muted small mb-3 font-weight-bold">' + theme.Currency.formatMoney(parseFloat(product.variants[0].price) * 100, theme.moneyFormat) + '</p>';
        html += '<button type="button" class="btn btn-sm btn-dark mt-auto" onclick="closeProductSelector(\'' + product.handle + '\')">Adaugă</button>';
        html += '</div></div></div>';
      ...
      if (addedCount === 0) {
        $list.html('<div class="col-12 text-center text-muted py-4">Nu mai există produse disponibile pentru comparare.</div>');
      }
    }).fail(function() {
      $list.html('<div class="col-12 text-center text-danger py-4">A apărut o eroare la încărcarea produselor.</div>');
    });
  };

  window.closeProductSelector = function(productHandle) {
    $('#jsProductSelector').removeClass('is-active');
    $('.modal-dialog--compare').removeClass('is-shrunk');
    
    if (productHandle) {
      if (compareObject.length < 4 && $.inArray(productHandle, compareObject) === -1) {
        compareObject.push(productHandle);
        localStorage.setItem('localCompare', JSON.stringify(compareObject));
        $compareCount.text(compareObject.length);
        loadCompare();
        theme.alert.new(theme.strings.compareText, theme.strings.compareNotifyAdded, 2000, 'notice');
      }
    }
  };
      ...
      <div class="col-6 col-lg-2 js-viewed-item mb-4">
        <div class="d-flex flex-column">
          <a class="w-100 recent-img ${imageSize}" href="${product.url}">
            <img class="lazyload" loading="lazy" data-src="${Shopify.resizeImage(product.featured_image, '340x')}"/>
          </a>
...
theme.alert = (function(){
  function createAlert(title,mess,time,type){
    ...
    if (typeof $.growl === 'function') {
      ...
      var $container = $('#growls-tr');
      if ($container.length === 0) {
        $container = $('<div id="growls-tr" style="position:fixed; top:20px; right:20px; z-index:99999;"></div>').appendTo('body');
      }
      var $growl = $('<div class="growl growl-large growl-' + aClass + ' growl-incoming" style="position:relative; margin-bottom:10px;"></div>');
      $growl.append('<div class="growl-close" style="position:absolute; right:10px; top:10px; cursor:pointer;">&times;</div>');
      if (aTitle) $growl.append('<div class="growl-title">' + aTitle + '</div>');
      $growl.append('<div class="growl-message">' + aMessage + '</div>');
      $container.append($growl);
      
      $growl.find('.growl-close').on('click', function() { $(this).parent().remove(); });
      setTimeout(function() { $growl.fadeOut(function(){ $(this).remove(); }); }, aTime);
    }
  }
...
/* Optimistic UI Loading State for All Add to Cart Buttons */
$(document).ready(function() {
  $(document).on('click', 'button[name="add"], .product-form__cart-submit, [class*="fast-bundle-add-to-cart"], [class*="fast-bundle-btn"], .add-to-cart, form[action^="/cart/add"] button[type="submit"]', function(e) {
    var $btn = $(this);
    if (!$btn.hasClass('btn-instant-loading')) {
      $btn.addClass('btn-instant-loading');
      // Remove it automatically after 2.5 seconds in case the main script doesn't redirect or clear it
      setTimeout(function() {
        $btn.removeClass('btn-instant-loading');
      }, 2500);
    }
  });
});
